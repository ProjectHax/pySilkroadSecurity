...
StreamUtility::StreamUtility() : m_stream(m_stream_internal)
{
	Clear();
}

StreamUtility::StreamUtility(std::vector< uint8_t > & stream) : m_stream(stream)
{
	m_read_index = 0;
	m_read_error = false;
	m_write_error = 0;
...
StreamUtility::StreamUtility(const void * const buffer, int32_t size) : m_stream(m_stream_internal)
{
	Clear();
	Overwrite< uint8_t >(0, reinterpret_cast< const uint8_t * >(buffer), size);
}

StreamUtility::StreamUtility(const StreamUtility & rhs) : m_stream(m_stream_internal), m_read_index (rhs.m_read_index), m_read_error(rhs.m_read_error)
{
...
int32_t StreamUtility::GetStreamSize() const
{
	return static_cast< int32_t >(m_stream.size());
}

int32_t StreamUtility::GetWriteIndex() const
{
...
int32_t StreamUtility::GetReadIndex() const
{
	return m_read_index;
}
...
std::string StreamUtility::Read_UnicodeToAscii(int32_t count)
{
	...
	std::use_facet<std::ctype<wchar_t> >(std::locale("")).narrow(str.c_str(), str.c_str() + count, '?', &mbsStr[0]);
...
void StreamUtility::Write_UnicodeToAscii(const wchar_t * wcs_text, int32_t count)
{
	...
	std::use_facet<std::ctype<wchar_t> >(std::locale("")).narrow(wcs_text, wcs_text + count, '?', &mbsStr[0]);
